...
function FuseLoading(props) {
  const customStyle = props.className;
  ...
  return (
    <div
      className={clsx(
        ...
        !showLoading && 'hidden', customStyle
      )}
    >
      <Typography className="text-13 sm:text-20 mb-16" color="textSecondary">
        Loading...
      </Typography>
      <LinearProgress className={clsx('w-192 sm:w-320 max-w-full rounded-2', customStyle)} color="secondary" />
    </div>
  );
}
